...
var Forever = function (file, options) {
  ...
  if (options.outFile) {
    this.stdout = fs.createWriteStream(options.outFile, { flags: 'w+', encoding: 'utf8', mode: 0666 });
    sys.log("output log -> "+options.outFile);
  }
  
  // If we should log stderr, open a file buffer
  if (options.errFile) {
    this.stderr = fs.createWriteStream(options.errFile, { flags: 'w+', encoding: 'utf8', mode: 0666 });
    sys.log("error log -> "+options.errFile);
  }
...
Forever.prototype.start = function (restart) {
  ...
  function listenTo (stream) {
    child[stream].on('data', function (data) {
      ...
      if ((stream == "stdout" && self.options.outFile) || (stream == "stderr" && self.options.errFile)) {
